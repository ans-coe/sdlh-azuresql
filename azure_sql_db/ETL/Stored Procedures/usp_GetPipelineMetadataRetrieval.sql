...
Created: 2022-11-01
Name: usp_GetPipelineMetadataRetrieval
Description:	 
   ...
   - @PARAM_DATETIME (DATETIME): Pipeline run start time. Pipeline dynamic expression: @pipeline().parameters.PARAM_DATETIME
   ...
   - @PARAM_OBJECT_NAME (STR): Object name corresponding to the current ForEach loop item. Pipeline dynamic expression: @item().OBJECT_NAME

    It returns the full information (metadata) about one single object from the batch. In addition to the metadata from ETL.JsonMetadata,
    ...
    This info is then used in the ingestion pipeline to drive the ETL, ETL logging, and queuing of the specific object within the 
    ForEach loop in the ingestion pipeline. 
    
    Fields returned:
    - All the fields from ETL.JsonMetadata in full (including JSON) for 1 single object
    - HNS 
    ...
    on matching SOURCE_TYPE, SOURCE_SYSTEM, SOURCE_GROUPING_ID, OBJECT_NAME and IS_ENABLED = 1.
...
===============================================================
*/
CREATE PROCEDURE [ETL].[usp_GetPipelineMetadataRetrieval]
...
BEGIN
    ...
    AND OBJECT_NAME = @PARAM_OBJECT_NAME
    AND IS_ENABLED = 1
END;