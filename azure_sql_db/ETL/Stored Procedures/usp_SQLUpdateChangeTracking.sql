...
Description:	 
    ...
    - @PARAM_NEW_CHANGE_TRACKING_VALUE (INT): Data retrived from a lookup to get the new change tracking value. New change tracking value. Pipeline dynamic expression: @activity('lookup_set_change_tracking_value').output.firstRow.NEW_CHANGE_TRACKING_VALUE
...
CREATE PROCEDURE [ETL].[usp_SQLUpdateChangeTracking]
    ...
    @PARAM_NEW_CHANGE_TRACKING_VALUE INT
AS

BEGIN
    UPDATE [ETL].[SQLTableMetadata]
    SET [LAST_CHANGE_TRACKING_VALUE] = @PARAM_NEW_CHANGE_TRACKING_VALUE
