...
BEGIN
    ...
    SELECT @PARAM_SOURCE_TYPE AS SOURCE_TYPE
        ...
        ,SOURCE_GROUPING_ID AS SOURCE_GROUPING_ID
        ...
        END AS OBJECT_NAME
        ,(SELECT distinct T.[SOURCE_SYSTEM],T.[SOURCE_GROUPING_ID],T.[SOURCE_CONNECTION_STRING],T.[DATABASE_NAME],T.[SCHEMA_NAME],T.[TABLE_NAME],T.[PRIMARY_KEYS],T.[SCHEMA_VERSION],T.[COLUMNS_META],T.[SERVERLESS_SQL_POOL_DATABASE],T.[SERVERLESS_SQL_POOL_SCHEMA],OBP.[INFORMATION_SCHEMA]
    ...
    AND source.SOURCE_SYSTEM = @PARAM_SOURCE_SYSTEM
        THEN
            INSERT (SOURCE_TYPE, SOURCE_SYSTEM, SOURCE_GROUPING_ID, IS_ENABLED, LOAD_TYPE, BATCH_FREQUENCY, OBJECT_NAME, OBJECT_PARAMETERS, ADLS_PATHS)
            VALUES (source.SOURCE_TYPE, source.SOURCE_SYSTEM, source.SOURCE_GROUPING_ID, source.IS_ENABLED, source.LOAD_TYPE, source.BATCH_FREQUENCY, source.OBJECT_NAME, source.OBJECT_PARAMETERS, source.ADLS_PATHS);
END;